...
# -------- Pre-reqs --------
echo "[1/5] Installing system prerequisites (python3-venv if available)..."
if command -v apt-get >/dev/null 2>&1; then
  $SUDO apt-get update -y
  $SUDO apt-get install -y python3-venv python3-pip
elif command -v dnf >/dev/null 2>&1; then
  $SUDO dnf install -y python3-virtualenv python3-pip || true
...
read -rp "HTTP API URL to send logs: " API_URL
if [[ -z "$API_URL" ]]; then
...
# -------- Env file --------
echo "[4/5] Writing environment file to $ENV_FILE ..."
...
# -------- Wrapper script --------
echo "[5/5] Creating wrapper script $WRAPPER ..."
cat > "$WRAPPER" <<'EOSH'
#!/usr/bin/env bash
set -euo pipefail
...
exec "${PROJECT_DIR}/venv/bin/python" "${PROJECT_DIR}/log_collector_daemon.py"
EOSH
chmod +x "$WRAPPER"
...
[Unit]
Description=Log Collector Daemon (forward logs to HTTP)
